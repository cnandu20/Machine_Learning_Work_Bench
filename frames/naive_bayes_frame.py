...
import pandas as pd
import  os
import Machine_Learning_Work_Bench.algorithm.naive_bayes_algorithm as nb
...
def nbframe():
    ...
    title = Label(lg, text="  NAIVE BAYES   ", font=20, bg='#141414', fg='white', height=2, ).place(x=75, y=10)
...
def train(path,feature_list,category_list,user):
  ...
  if((len(feature_list.curselection())>0) & (len(category_list.curselection()))>0):
      ...
      score,classifier,feature_list_index = nb.nb_algorithm(path,feature_list,category_list)
      if(score):
